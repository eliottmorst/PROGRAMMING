...
int start()
{
  ...
  {
     int result=0;
     if((iSAR(NULL,0,0.02,0.2,0)<Close[0])) // Here is your open buy rule
     {
     ...
     if((iSAR(NULL,0,.02,.2,0)>Close[0])) // Here is your open Sell rule
     {
  ...
  for(int cnt=0;cnt<OrdersTotal();cnt++)
     {
      ...
      if(OrderType()<=OP_SELL &&   
         ...
         )  
        {
         if(OrderType()==OP_BUY)  
           {
              if((iSAR(NULL,0,0.02,.2,0)>Close[0])) //here is your close buy rule
              {
           ...
           {
                if((iSAR(NULL,0,0.02,.2,0)<Close[0])) // here is your close sell rule
                {
                   OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),Slippage,Red);
                }
            if(TrailingStop>0)  
